...
API_TOKEN = "1440758165:VRHkKeefrKH7diHZHASsOmLrUPLpSPgHNjM"
ADMIN_ID = 000
CHANNEL_ID = "@test_village"
...
def start(message):
    if is_group_chat(message):
        ...
        if user:
            ...
            markup.add(
                types.InlineKeyboardButton("🙌 بیانیه", callback_data="statement")
            )
            markup.add(
                types.InlineKeyboardButton(
                    "✉️ پیام خصوصی", callback_data="private_message"
                )
            )
            ...
            if user_id == ADMIN_ID:
                markup.add(
                    types.InlineKeyboardButton(
                        "🔨آپ هفتگی", callback_data="weekly_update"
                    )
                )
                markup.add(
                    types.InlineKeyboardButton(
                        "🛠️ تنظیم دارایی", callback_data="change_assets"
                    )
                )
...
def show_assets(message):
    ...
    if user:
        assets_message = (
            ...
            f"🍞 _غذا_ : {user[6]}\n"
            f"🍖 _گوشت_ : {user[7]}\n"
            f"🗡️ _سرباز شمشیرزن_ : {user[8]}\n"
            f"🔫 _سرباز تفنگدار_ : {user[9]}\n"
            f"🗡️ _سواره‌نظام شمشیرزن_ : {user[10]}\n"
            f"🔫 _سواره‌نظام تفنگدار_ : {user[11]}\n"
            f"🛡️ _گارد ویژه_ : {user[12]}\n"
            f"🎯 _توپ متوسط_ : {user[13]}\n"
            f"🎯 _توپ بزرگ_ : {user[14]}\n"
            f"⛵ _کشتی کوچک_ : {user[15]}\n"
            f"🚢 _کشتی متوسط_ : {user[16]}\n"
            f"🛳️ _کشتی بزرگ_ : {user[17]}</blockquote>\n\n"
            f"🏭 _کارخانجات_ :\n"
...
def show_upgrade_options(message):
    ...
    markup.add(
        types.InlineKeyboardButton("کارخونه سنگ", callback_data="upgrade_stone_factory")
    )
    markup.add(
        types.InlineKeyboardButton("کارخونه چوب", callback_data="upgrade_wood_factory")
    )
    markup.add(
        types.InlineKeyboardButton("کارخونه آهن", callback_data="upgrade_iron_factory")
    )
    markup.add(
        types.InlineKeyboardButton("معدن طلا", callback_data="upgrade_gold_mine")
    )
    markup.add(
        types.InlineKeyboardButton("زمین کشاورزی", callback_data="upgrade_farm_farm")
    )
    markup.add(
        types.InlineKeyboardButton("دامداری", callback_data="upgrade_animal_farm")
    )
    markup.add(
        types.InlineKeyboardButton(
            "کارخانه لباس", callback_data="upgrade_clothes_factory"
        )
    )
    markup.add(types.InlineKeyboardButton("بانک", callback_data="upgrade_bank_bank"))
    markup.add(
        types.InlineKeyboardButton(
            "کمپ سرباز شمشیرزن", callback_data="upgrade_swordsmen_camp"
        )
    )
    markup.add(
        types.InlineKeyboardButton(
            "کمپ سرباز تفنگدار", callback_data="upgrade_gunmen_camp"
        )
    )
    markup.add(
        types.InlineKeyboardButton(
            "کمپ سواره‌نظام شمشیرزن", callback_data="upgrade_cavalryswordsmen_camp"
        )
    )
    markup.add(
        types.InlineKeyboardButton(
            "کمپ سواره‌نظام تفنگدار", callback_data="upgrade_cavalrygunmen_camp"
        )
    )
    markup.add(
        types.InlineKeyboardButton(
            "کمپ گارد ویژه", callback_data="upgrade_specialguard_camp"
        )
    )
    markup.add(
        types.InlineKeyboardButton(
            "کارخانه توپ متوسط", callback_data="upgrade_mediumcannon_factory"
        )
    )
    markup.add(
        types.InlineKeyboardButton(
            "کارخانه توپ بزرگ", callback_data="upgrade_largecannon_factory"
        )
    )
    markup.add(
        types.InlineKeyboardButton(
            "کشتی‌سازی کوچک", callback_data="upgrade_small_shipyard"
        )
    )
    markup.add(
        types.InlineKeyboardButton(
            "کشتی‌سازی متوسط", callback_data="upgrade_medium_shipyard"
        )
    )
    markup.add(
        types.InlineKeyboardButton(
            "کشتی‌سازی بزرگ", callback_data="upgrade_large_shipyard"
        )
    )
    bot.send_message(
        message.chat.id,
        "انتخاب کنید که کدام بخش را میخواهید ارتقا دهید",
        reply_markup=markup,
    )
...
def confirm_upgrade(message):
    ...
    markup.add(
        types.InlineKeyboardButton(
            "بله", callback_data=f"upgrade_confirm_{item_to_upgrade}"
        )
    )
...
def get_upgrade_cost_message():
    if item_to_upgrade == "stone_factory":
        ...
        return (
            "برای ارتقا کمپ سواره‌نظام تفنگدار به 800 طلا، 800 سنگ و 500 پول نیاز دارید."
        )
...
def check_upgrade_cost(group_id):
    cursor.execute(
        "SELECT stones, wood, iron, gold, money FROM users WHERE group_id=?",
        (group_id,),
    )
...
def ask_for_private_message(message, user_id):
    ...
    bot.register_next_step_handler(
        message, lambda msg: get_private_message(msg, user_id)
    )
...
def get_private_message(message, user_id):
    ...
    if groups:
        ...
        for group in groups:
            ...
            markup.add(
                types.InlineKeyboardButton(
                    bot_name, callback_data=f"private_send_{group[0]}"
                )
            )
        bot.send_message(
            message.chat.id,
            "انتخاب کنید که به کدام گروه ارسال شود:",
            reply_markup=markup,
        )
...
def show_asset_change_options(message):
    ...
    for asset in asset_types:
        markup.add(
            types.InlineKeyboardButton(asset, callback_data=f"change_asset_{asset}")
        )
    bot.send_message(
        message.chat.id,
        "انتخاب کنید که کدام دارایی را میخواهید تغییر دهید:",
        reply_markup=markup,
    )
...
def ask_for_new_asset_value(message, asset_type):
    ...
    bot.send_message(
        message.chat.id, f"لطفا مقدار جدید برای {asset_type} را وارد کنید:"
    )
    bot.register_next_step_handler(
        message, lambda msg: set_new_asset_value(msg, group_id)
    )
...
def set_new_asset_value(message, group_id):
    try:
        ...
        cursor.execute(
            f"UPDATE users SET {asset_type} = ? WHERE group_id = ?",
            (new_value, group_id),
        )
        conn.commit()
        ...
        bot.send_message(
            message.chat.id, "مقدار وارد شده معتبر نیست. لطفا یک عدد وارد کنید."
        )
...
def show_treaty_options(message):
    ...
    markup.add(
        types.InlineKeyboardButton("ثبت معاهده جدید", callback_data="treaty_new")
    )
    bot.send_message(
        message.chat.id,
        "انتخاب کنید که کدام بخش را میخواهید انجام دهید",
        reply_markup=markup,
    )
...
def ask_for_treaty_content(message, user_id):
    ...
    bot.register_next_step_handler(
        message, lambda msg: get_treaty_content(msg, user_id)
    )
...
def get_treaty_content(message, user_id):
    ...
    if groups:
        ...
        for group in groups:
            ...
            markup.add(
                types.InlineKeyboardButton(
                    bot_name, callback_data=f"treaty_send_{group[0]}"
                )
            )
        bot.send_message(
            message.chat.id,
...
def send_treaty_confirmation(call, group_id):
    ...
    if treaty_content:
        ...
        markup.add(
            types.InlineKeyboardButton("خیر", callback_data="treaty_not_confirmed")
        )
        user_info = bot.get_chat(user_id)
        user_name = f"<a href='tg://user?id={user_id}'>{user_info.first_name}</a>"
        bot.send_message(
            group_id,
            f"📜 معاهده جدید از {user_name}:\n\n{treaty_content}\n\nتایید میکنید؟",
            reply_markup=markup,
            parse_mode="HTML",
        )
...
def process_treaty_confirmation(call):
    ...
    if call.data == "treaty_confirmed":
        ...
        new_treaties = (
            ...
            if user_treaties
            else user_context[user_id].get("treaty_content")
        )
        cursor.execute(
            "UPDATE users SET treaties = ? WHERE user_id = ?", (new_treaties, user_id)
        )
        conn.commit()
    ...
    if user_factories:
        ...
        cursor.execute(
            ...
            (
                ...
                meat_collected,
                clothes_collected,
                small_ships_collected,
                medium_ships_collected,
                large_ships_collected,
                swordsmen_collected,
                gunmen_collected,
                cavalry_swordsmen_collected,
                cavalry_gunmen_collected,
                special_guard_collected,
                medium_cannons_collected,
                large_cannons_collected,
                group_id,
            ),
        )
        conn.commit()
...
def ask_for_statement(message, user_id):
    bot.send_message(
        message.chat.id, "<b>لطفا بیانیه خود را ارسال کنید </b>", parse_mode="HTML"
    )
...
def send_statement(message, user_id):
    ...
    if message.text:
        bot.send_message(
            CHANNEL_ID, f"{message.text}{additional_caption}", parse_mode="HTML"
        )
    elif message.photo:
        ...
        bot.send_photo(
            ...
            caption=f"{original_caption}{additional_caption}",
            parse_mode="HTML",
        )
    elif message.video:
        original_caption = message.caption if message.caption else " "
...
def ask_for_attack_type(message):
    ...
    bot.send_message(
        message.chat.id, "نوع لشکرکشی را انتخاب کنید:", reply_markup=markup
    )
...
def handle_attack_type_selection(call):
    ...
    bot.register_next_step_handler(
        call.message, lambda msg: get_attack_origin(msg, user_id)
    )
...
def get_attack_origin(message, user_id):
    ...
    bot.register_next_step_handler(
        message, lambda msg: get_attack_destination(msg, user_id)
    )
...
def get_attack_time(message, user_id):
    ...
    bot.register_next_step_handler(
        message, lambda msg: send_attack_details(msg, user_id)
    )
...
def send_attack_details(message, user_id):
    ...
    bot.send_photo(
        CHANNEL_ID,
        photo_url,
        caption=f"🔖 ارتش کشور {attack_origin} به مقصد {attack_destination} حرکت کردند ({attack_type})\n\n⚜️ "
        f"فرمانده: {user_name}\n⌛️ زمان رسیدن: {attack_time}",
        parse_mode="HTML",
    )
